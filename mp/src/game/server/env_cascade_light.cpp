...
ConVar cl_csm_auto_world("cl_csm_auto_world", "1", 0, "");
...
bool CCascadeLight::GetKeyValue(const char* szKeyName, char* szValue, int iMaxLen)
{
	if (FStrEq(szKeyName, "color"))
	{
		//FIXME: error C2039L 'r': is not a member of 'color32'
		//Q_snprintf(szValue, iMaxLen, "%d %d %d %d", m_LightColor.r(), m_LightColor.g(), m_LightColor.b(), m_LightColor.a());
		return true;
	}
